"""Tests for the M3 serving core (Methodology M3).

The serving path (make_predictor + dev-engine scaler + shared window builder)
must reproduce the M3 freeze predictions exactly, and the engine-cluster
conformal interval + objective history fields must match the M3
calibration.
...
Requires gitignored artifacts (models/, data/raw, experiments/m3) — excluded
...
from rul_prediction.serving.m1_predictor import (
    M1Predictor,
...
def test_engine_cluster_conformal_interval_and_padding_fields():
    ...
    assert predictor.model_version.startswith("m3-")
