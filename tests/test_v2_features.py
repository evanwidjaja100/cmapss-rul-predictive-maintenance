"""Methodology V2 feature engineering tests (artifact-free)."""
...
def test_feature_count_and_names():
    f, names = extract_v2_features(_observed(30), cutoff_cycle=60)
...
def test_no_future_or_full_history_dependence():
    ...
    full = _observed(50)
    f_full, _ = extract_v2_features(full[:30], cutoff_cycle=30)
    f_obs, _ = extract_v2_features(full[:30].copy(), cutoff_cycle=30)
    np.testing.assert_array_equal(f_full, f_obs)


def test_short_history_no_nan_and_fallbacks():
    for n in (1, 2, 4, 5, 9, 10, 11):
        f, _ = extract_v2_features(_observed(n), cutoff_cycle=7)
        assert not np.isnan(f).any()
        assert f[-1] == n  # history_length


def test_engine_age_is_cutoff_cycle():
    f1, _ = extract_v2_features(_observed(30), cutoff_cycle=60)
    f2, _ = extract_v2_features(_observed(30), cutoff_cycle=200)
    assert f1[-2] == 60.0 and f2[-2] == 200.0


def test_deterministic():
    a, _ = extract_v2_features(_observed(30), cutoff_cycle=60)
    b, _ = extract_v2_features(_observed(30), cutoff_cycle=60)
    np.testing.assert_array_equal(a, b)


def test_single_row_history():
    f, names = extract_v2_features(np.ones((1, 3)), cutoff_cycle=1)
    assert f[-1] == 1.0
...
def test_classical_manifest_pipeline():
    ...
    preds = evaluate_manifest(
        ...
        lambda history, cutoff: float(
            model.predict(extract_v2_features(
