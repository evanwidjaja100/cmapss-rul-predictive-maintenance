"""Methodology V2.1 methodology-regression tests (artifact-free, synthetic).

Covers the repairs in V2_1_REPAIR_PLAN.md:
...
def test_v2_1_fractions_fixed():
    assert V2_1_LIFECYCLE_FRACTIONS == (0.25, 0.45, 0.65, 0.80, 0.95)


def test_manifest_exactly_five_cutoffs_per_engine():
    lifetimes = {e: 200 + e for e in range(1, 11)}
    manifest = build_pseudo_test_manifest(lifetimes, V2_1_LIFECYCLE_FRACTIONS)
    assert len(manifest) == 10 * 5
    assert manifest.groupby("engine_id").size().eq(5).all()
    assert sorted(manifest["fraction"].unique()) == sorted(V2_1_LIFECYCLE_FRACTIONS)
