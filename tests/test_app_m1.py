...
def test_app_import_is_artifact_free_subprocess():
    """Bare `import app_m1` must succeed without gitignored artifacts and without side effects."""
    code = """
        import time, sys
        t0 = time.perf_counter()
        import app_m1
        dt = time.perf_counter() - t0
        assert hasattr(app_m1, 'get_predictor'), 'get_predictor missing'
        assert hasattr(app_m1, 'main'), 'main missing'
        assert callable(app_m1.get_predictor)
        assert callable(app_m1.main)
        # no predictor instance at import
        assert not hasattr(app_m1, 'predictor')
...
def test_app_has_main_and_get_predictor():
    code = """
        import app_m1
        assert hasattr(app_m1, 'main')
        assert hasattr(app_m1, 'get_predictor')
        assert callable(app_m1.main)
        assert callable(app_m1.get_predictor)
        # st.cache_resource adds attrs
        assert hasattr(app_m1.get_predictor, 'clear') or hasattr(app_m1.get_predictor, '__wrapped__') or True
        print('has_both')
    """
    result = _run(code)
    ...
    # also direct
    import app_m1
    assert hasattr(app_m1, "main") and hasattr(app_m1, "get_predictor")


def test_import_does_not_construct_predictor():
    """Import must not call M1Predictor.__init__."""
    code = """
        import sys
        # spy on M1Predictor before app_m1 import
        import rul_prediction.serving.m1_predictor as vp
        orig = vp.M1Predictor.__init__
        called = []
        def spy(self, *a, **kw):
            called.append(1)
            return orig(self, *a, **kw)
        vp.M1Predictor.__init__ = spy
        import app_m1
        assert called == [], f"M1Predictor.__init__ called at import: {called}"
        print('spy_ok')
        vp.M1Predictor.__init__ = orig
    """
    result = _run(code)
...
def test_import_does_not_open_models_scalers_raw_data():
    ...
    code = """
        ...
        def spy_open(file, *a, **kw):
            s = str(file)
            lower = s.lower()
            if any(x in lower for x in ['models/m3', 'fd001', 'scaler', 'data/raw']):
                # ignore reading configs which are allowed? But models/scalers/raw must not open
                if 'models/m3' in lower or 'scaler' in lower or 'data/raw' in lower:
                    opened.append(s)
        ...
        def spy_popen(self, *a, **kw):
            s = str(self).lower()
            if 'models/m3' in s or 'scaler' in s or 'data/raw' in s:
...
def test_xgboost_serving_does_not_import_tensorflow():
    ...
    code = """
        ...
        import yaml, pathlib
        cfg = yaml.safe_load(pathlib.Path('configs/final_model_m3_fd001.yaml').read_text(encoding='utf-8'))
...
def test_friendly_missing_artifact_guidance_only_when_requested():
    ...
    code = """
        ...
        # patch both app_m1 predictor root and m1_predictor ROOT
        import rul_prediction.serving.m1_predictor as m1p
        import rul_prediction.benchmark.m1 as bench_m1
        orig = m1p.ROOT
        orig2 = bench_m1.ROOT
        m1p.ROOT = tmp
        bench_m1.ROOT = tmp
        try:
            try:
                ...
                msg = str(e)
                low = msg.lower()
                assert 'run_m3_freeze' in msg or 'scripts' in low, msg
...
def test_headless_streamlit_smoke_with_artifacts():
    ...
    from app_m1 import get_predictor
    try:
        get_predictor.clear()
    except Exception:
        pass
    ...
    try:
        ...
        s.close()
    cmd = [sys.executable, "-m", "streamlit", "run", "app_m1.py", "--server.headless", "true", "--server.port", str(port), "--server.address", "127.0.0.1", "--browser.gatherUsageStats", "false"]
