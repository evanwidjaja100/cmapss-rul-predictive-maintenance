...
def _production_mapping():
    return yaml.safe_load((ROOT / "configs" / "final_model_m3_fd004.yaml").read_text(encoding="utf-8"))
...
def test_production_yaml_parses_to_expected_contract():
    cfg = load_fd004_final_config(ROOT / "configs" / "final_model_m3_fd004.yaml")
    ...
    assert cfg.split_provenance == "experiments/splits/FD004_m1_seed42.json"
    assert cfg.validation_manifest == "experiments/splits/fd004_m2_validation_cutoffs.csv"
...
def test_non_default_config_reaches_all_runtimes_via_spies(monkeypatch, tmp_path):
    ...
    import scripts.run_m3_fd004 as runner
    ...
    import scripts.run_m3_fd004_freeze as freeze_mod
    ...
    # spy m1_gru
    import rul_prediction.models.m1_models as vm
    orig_gru = vm.m1_gru
    ...
    monkeypatch.setattr(vm, "m1_gru", spy_gru)
    monkeypatch.setattr(freeze_mod, "m1_gru", spy_gru)

    # spy build_m1_train_sequences
    import rul_prediction.data.m1_preprocessing as m1p
    orig_seq = m1p.build_m1_train_sequences

    def spy_seq(scaled, engine_ids, rul, window):
        captured["seq_window"] = window
        return orig_seq(scaled, engine_ids, rul, window)

    monkeypatch.setattr(m1p, "build_m1_train_sequences", spy_seq)
    monkeypatch.setattr(freeze_mod, "build_m1_train_sequences", spy_seq)  # freeze imports directly? it imports add_raw_rul etc but calls build_m1_train_sequences via import
    # need to patch where it's imported
    import rul_prediction.data.m1_preprocessing as m1p2
    monkeypatch.setattr(freeze_mod, "build_m1_train_sequences", spy_seq, raising=False)
    ...
    from scripts.run_m3_fd004_freeze import fit_fd004_final_model
...
def test_freeze_and_posthoc_resolve_identical_artifact_paths():
    cfg = load_fd004_final_config(ROOT / "configs" / "final_model_m3_fd004.yaml")
    # freeze paths
    from scripts.run_m3_fd004_freeze import load_and_validate_split  # just to ensure import works
    ...
    assert str(freeze_model.relative_to(ROOT)).replace("\\", "/") == "models/m3/fd004_gru_w45_huber_condC.keras"
    assert str(freeze_cond.relative_to(ROOT)).replace("\\", "/") == "models/m3/fd004_conditionC.joblib"
...
def test_temporary_split_paths_consumed_and_drift_fails(tmp_path):
    ...
    cfg = from_mapping(mapping)
    from scripts.run_m3_fd004_freeze import load_and_validate_split
...
def test_historical_legacy_payload_byte_identical_and_gated(tmp_path, monkeypatch):
    historical_path = ROOT / "models" / "m3" / "fd004_conditionC.joblib"
    ...
    from scripts.run_m3_fd004_posthoc import load_and_verify_condition
...
def test_future_condition_payload_is_not_loaded_without_manifest_authorization(tmp_path, monkeypatch):
    cfg = load_fd004_final_config(ROOT / "configs" / "final_model_m3_fd004.yaml")
    from rul_prediction.benchmark.fd004_config import FD004FinalConfig
    import scripts.run_m3_fd004_posthoc as posthoc
...
def test_config_metadata_shape_disagreement_fails(tmp_path):
    ...
    from scripts.run_m3_fd004_posthoc import _verify_model_dimensions
    with pytest.raises(FD004ConfigError):
...
def test_resume_idempotency_preserves_prior_rows(tmp_path, monkeypatch):
    ...
    # Now simulate merging logic as in run_m3_fd004 main: existing with A, new with B
    # Use helper from run_m3_fd004 (merge logic) – we replicate
...
def test_partial_results_cannot_overwrite_canonical_config(tmp_path):
    ...
    from scripts.run_m3_fd004 import write_fd004_config
    with pytest.raises(ValueError):
        write_fd004_config(winner, results, best_rows, "configs/final_model_m3_fd004.yaml")
    ...
    canonical_path = ROOT / "configs" / "final_model_m3_fd004.yaml"
...
def test_canonical_guard_bypass_via_absolute_path(tmp_path):
    ...
    winner = results.iloc[0]
    absolute_spelling = str(ROOT / "configs" / "final_model_m3_fd004.yaml")
    with pytest.raises(ValueError):
        write_fd004_config(winner, results, best_rows, absolute_spelling)
    # and the canonical file is untouched
    assert (ROOT / "configs" / "final_model_m3_fd004.yaml").exists()
...
def test_save_refuses_real_historical_condition_baseline(tmp_path, monkeypatch):
    ...
    real_cond = ROOT / "models" / "m3" / "fd004_conditionC.joblib"
...
def test_runner_main_rejects_dirty_execution(monkeypatch):
    ...
    import scripts.run_m3_fd004 as runner
    import scripts.run_m3_fd004_freeze as freeze_mod
    import scripts.run_m3_fd004_posthoc as posthoc_mod
