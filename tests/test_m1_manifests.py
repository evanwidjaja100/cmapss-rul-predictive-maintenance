"""Methodology M1: split + pseudo-test manifest + manifest evaluation tests."""
...
def test_m1_split_deterministic():
    a = split_engine_ids_m1(ENGINES, seed=SEED)
    b = split_engine_ids_m1(ENGINES, seed=SEED)
    assert a == b


def test_m1_split_no_overlap():
    train, validation, calibration = split_engine_ids_m1(ENGINES, seed=SEED)
    assert train.isdisjoint(validation)
    assert train.isdisjoint(calibration)
    assert validation.isdisjoint(calibration)


def test_m1_split_counts_and_union():
    train, validation, calibration = split_engine_ids_m1(ENGINES, seed=SEED)
    assert len(train) == 70
...
def test_m1_split_write_roundtrip(tmp_path):
    path = write_m1_split_file(ENGINES, "FD001", tmp_path, seed=SEED)
    assert path.name == "FD001_m1_seed42.json"
    train, validation, calibration = read_m1_split_file(path)
    assert len(train) == 70 and len(validation) == 15 and len(calibration) == 15
    assert train | validation | calibration == ENGINES


def test_m1_split_fd004_magnitudes():
    """Phase M1-11: FD004 split magnitudes on the 249-engine cardinality."""
...
def test_m1_manifest_fd004_count():
    """Phase M1-11: FD004 manifest = 5 lifecycle fractions per engine."""
