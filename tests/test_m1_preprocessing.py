"""Methodology M1 raw-RUL preprocessing tests (artifact-free)."""
...
def test_train_sequences_use_same_builder_as_inference():
    ...
    X, y, ids, obs, masks = build_m1_train_sequences(scaled, frame["engine_id"].to_numpy(),
...
def test_train_sequences_raw_targets_uncapped():
    ...
    X, y, ids, obs, masks = build_m1_train_sequences(
...
def test_partition_leakage_guard_in_script_contract():
    ...
    # in the M1 split contract (disjointness is asserted by splitting module).
    from rul_prediction.data.splitting import split_engine_ids_m1
    train, validation, calibration = split_engine_ids_m1(set(range(1, 101)), seed=42)
