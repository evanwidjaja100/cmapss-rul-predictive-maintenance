...
def _copy_bundle(tmp: Path) -> Path:
    ...
    # Also copy manifest files themselves (already under experiments/m3)
    return tmp
...
def test_absolute_and_traversal_rejected():
    ...
    bad = {
        ...
        "config_integrity": {"path": "configs/final_model_m3_fd001.yaml", "config_file_sha256": "a"*64, "config_canonical_sha256": "b"*64, "canonical_algo": "x", "note": ""},
...
def test_reject_wrong_identity():
    ...
    bad["artifacts"].append({"role": "variant_results", "path": "experiments/m3/fd004_variant_results.csv", "sha256": "a"*64, "bytes": 1, "storage_class": "git", "required_in_clean_clone": True, "hash_kind": "raw_sha256"})
...
def test_preserve_generated_at_when_inputs_unchanged():
    ...
    try:
        ...
        subprocess.run([sys.executable, "scripts/build_m3_artifact_manifests.py", "--dataset", "FD001"], check=True, cwd=str(REPO_ROOT))
...
def test_check_mode_does_not_rewrite():
    ...
    res = subprocess.run([sys.executable, "scripts/build_m3_artifact_manifests.py", "--check", "--dataset", "FD001"], capture_output=True, text=True, cwd=str(REPO_ROOT))
...
def test_verify_tracked_passes_clean_clone_without_local():
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_clean_") as td:
        ...
        for rel in ["models/m3/fd001_xgb_w90_d6.joblib", "models/m3/fd001_scaler.joblib", "models/m3/fd004_gru_w45_huber_condC.keras", "models/m3/fd004_conditionC.joblib"]:
            p = tmp / rel
            if p.exists():
                p.unlink()
...
def test_tamper_temp_bundle_fails_before_deserialization():
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_tamper_") as td:
        ...
        real_path = REPO_ROOT / "experiments/m3/fd001_cv_summary.csv"
        real_hash_before = sha256_file(real_path)
        # tamper temp bundle file
        target = tmp / "experiments/m3/fd001_cv_summary.csv"
        ...
        with pytest.raises(ArtifactHashMismatchError):
            verify_before_load("experiments/m3/fd001_cv_summary.csv", root=tmp, manifest_dataset="FD001")
...
def test_missing_local_tracked_permitted_full_fails():
    with tempfile.TemporaryDirectory(prefix="bundle_missing_") as td:
        ...
        _copy_bundle(tmp)
        # remove one local
        p = tmp / "models/m3/fd001_xgb_w90_d6.joblib"
        assert p.exists()
        p.unlink()
        ...
        with pytest.raises(ArtifactMissingError):
            verify_before_load("models/m3/fd001_xgb_w90_d6.joblib", root=tmp, manifest_dataset="FD001")
...
def test_present_wrong_local_fails_even_in_tracked():
    with tempfile.TemporaryDirectory(prefix="bundle_wrong_") as td:
        ...
        model_path = tmp / "models/m3/fd001_xgb_w90_d6.joblib"
        scaler_path = tmp / "models/m3/fd001_scaler.joblib"
        # ensure they differ
...
def test_builder_and_verifier_accept_root_override():
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_root_") as td:
        ...
        subprocess.run([sys.executable, str(REPO_ROOT / "scripts/verify_m3_artifacts.py"), "--mode", "tracked", "--root", str(tmp), "--dataset", "FD001"], check=True)
        ...
        tamper_target = tmp / "experiments/m3/fd001_cv_summary.csv"
    ...
    with tempfile.TemporaryDirectory(prefix="builder_root_") as td2:
        ...
        shutil.copytree(REPO_ROOT / "models" / "m3", clone / "models" / "m3", dirs_exist_ok=True)
        subprocess.run([sys.executable, str(REPO_ROOT / "scripts/build_m3_artifact_manifests.py"), "--root", str(clone), "--generated-at", "2026-08-21T00:00:00Z", "--dataset", "FD001"], check=True)
...
def test_cli_check_is_deterministic_and_does_not_write():
    ...
    res = subprocess.run([sys.executable, "scripts/build_m3_artifact_manifests.py", "--check", "--dataset", "FD004"], capture_output=True, text=True, cwd=str(REPO_ROOT))
    assert res.returncode == 0, res.stdout + res.stderr
...
def test_load_time_verification_distinct_error_classes():
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_absent_") as td:
        ...
        (tmp / "configs/final_model_m3_fd001.yaml").unlink()
        with pytest.raises(ArtifactMissingError) as exc:
            verify_before_load("configs/final_model_m3_fd001.yaml", root=tmp, manifest_dataset="FD001")
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_mismatch_") as td:
        ...
        tmp.mkdir()
        _copy_bundle(tmp)
        p = tmp / "configs/final_model_m3_fd001.yaml"
    ...
    with tempfile.TemporaryDirectory(prefix="bundle_legacy_") as td:
        ...
        with pytest.warns(UserWarning, match="UNVERIFIED legacy-path load"):
            verify_before_load("models/m3/fd001_xgb_w90_d6.joblib", root=tmp, manifest_dataset="FD001")  # should not raise if manifest missing
...
def test_load_time_verification_normalizes_windows_text_line_endings(tmp_path):
    manifest_dir = tmp_path / "experiments" / "m3"
    ...
    config = (REPO_ROOT / "configs" / "final_model_m3_fd004.yaml").read_bytes()
    crlf = config.replace(b"\r\n", b"\n").replace(b"\n", b"\r\n")
    (config_dir / "final_model_m3_fd004.yaml").write_bytes(crlf)
    verify_before_load(
