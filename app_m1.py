"""Streamlit serving app for the frozen M3 model (Methodology M3).

Run:  .venv\\Scripts\\python.exe -m streamlit run app_m1.py

Serves the deployment model selected by the pre-specified M3 policy
(configs/final_model_m3_fd001.yaml) with the engine-cluster conformal
interval (q from the tracked deployment config
configs/deployment_m3_fd001.yaml). Shows model version, predicted raw RUL,
...
@st.cache_resource
def get_predictor():
    # ponytail: lazy import inside function, not at module import; keeps `import app_m1` artifact-free
...
def main() -> None:
    ...
    st.caption(
        f"Methodology M3 deployment model: {predictor.model_version}, window "
        ...
        "Post-hoc: official test labels were inspected in the M1-0 audit."
    )
    st.info(predictor.uncertainty_disclosure)

    with st.sidebar:
        st.header("Method notes (M3)")
        st.write(
            ...
            "input, not OOD.\n"
            f"- Model: `models/m3/fd001_{predictor.candidate}.{'joblib' if predictor._model_name in ('rf','xgboost') else 'keras'}`; "
            "CV + selection + freeze in reports/m3_final_report.md."
        )
    ...
    if mode.startswith("Demo"):
        ...
        warning = __import__("rul_prediction.serving.m1_predictor", fromlist=["limited_history_warning"]).limited_history_warning(int(result["n_cycles_observed"]), predictor.window)
        if warning:
            st.warning(warning)
        ...
        st.subheader("Top M3 sensitivity sensors")
        ...
        if uploaded is not None:
            ...
            st.download_button("Download predictions CSV", csv, "m3_predictions.csv", "text/csv")
