...
Model: `gru_w45_huber` (V2.1 CV-selected, 85 development engines). Official test: 100 engines. Total NASA = 6700.59.

**Terminology correction (V2_1_REPAIR_PLAN.md R1/R2):** official test trajectories are truncated before failure; `cycle.max()` = observed history length, NOT failure lifetime. `implied_failure_cycle = observed_cycles + true_rul`. `error = prediction - true_rul` (positive = overprediction).
