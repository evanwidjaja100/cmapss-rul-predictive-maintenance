# M2 Cross-Validation and Model Selection (FD001)
...
- **Development / calibration separation**: 100 FD001 engines → 85 development
  ...
  sha256 prefix `2b6229c9ef25`, preserved from the M1 split — used only for
...
- **5-fold engine-group CV, seed 42**: 68 train / 17 validation engines per
  ...
  Manifest: `experiments/splits/fd001_m2_group_cv_seed42.json`.
- **Lifecycle fractions** (fixed pre-comparison): 0.25 / 0.45 / 0.65 / 0.80 /
  0.95 → 85 manifest rows per fold (`fd001_m2_fold{k}_validation_cutoffs.csv`).
...
- **Candidates (8 ≤ 12 bound)**: gru/lstm w45+w60 huber, rf/xgb w60+w90.
  Excluded with reason: TCN (never competitive in M1-4, ~2x cost), linear
  (unusable on raw RUL, M1-3).

## Results (experiments/m2/fd001_cv_summary.csv, fd001_cv_fold_results.csv,
...
Artifacts: `models/m2/fd001_gru_w45_huber.keras`,
`models/m2/fd001_scaler.joblib`,
`reports/tables/m2_fd001_official.csv`, `m2_fd001_predictions.csv`.
