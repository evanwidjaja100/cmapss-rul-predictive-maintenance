...
3. **M1-8 (conformal calibration) should target the lower prediction bound** — the early-warning side — and be assessed with an asymmetric/conservative score, not just interval coverage.
4. A conservative bias correction (shift predictions down) is the cheapest mitigation for the frozen model; its cost is a small RMSE loss, its gain is collapsing the NASA tail. Quantify before adopting (M1-8).
5. Young-failing engines (lifetime ≲ 110 cycles) deserve feature-level scrutiny in M1-7 (SHAP) — they carry most of the risk.

Next: **Phase M1-7 — explainability (SHAP)** on the frozen model (currently shap is not installed; the phase will add it to the venv and requirements).