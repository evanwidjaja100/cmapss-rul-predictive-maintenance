...
- **Golden-file test** (`tests/test_inference_golden.py`): serving predictions over the full official test file match the Phase 9 one-time evaluation (`FD001_test_predictions.csv`) per unit within 1e-4. This is the anti-regression guarantee that serving never drifts from the frozen evaluation.
