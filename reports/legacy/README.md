...
| `reports/phase8_ablation.md` | Cap sweep that selected 45 (target-range-changing ablation; not repeated in V2) |
| `reports/phase9_final_evaluation.md` | Official FD001 test evaluation against clipped truth |
| `reports/phase10_serving.md` | HTTP/batch serving + golden-file verification (service retained in V2) |
...
## Status in Methodology V2

- V2 primary target: **raw RUL** (`raw_rul = max_cycle - current_cycle`, no cap).
- The cap-45 task is retained only as a clearly separated **secondary
  maintenance-horizon experiment**. Its metrics are never mixed into V2
  benchmark tables.
