"""Methodology M1 sequence models: masking-aware LSTM / GRU / TCN.

M1 training includes left-padded short-history windows (shared history
...
def m1_lstm(window: int, n_features: int, units=(128, 64), dropout: float = 0.3,
...
def m1_gru(window: int, n_features: int, units=(128, 64), dropout: float = 0.3,
           loss: str = "mse", learning_rate: float = 1e-3, seed: int = 42, clipnorm: float = 1.0):
    from tensorflow import keras
...
def m1_tcn(window: int, n_features: int, filters: int = 64, dilations=(1, 2, 4, 8),
