"""Methodology V2.1 development/calibration separation and engine-group CV.

V2.1 repairs (see V2_1_REPAIR_PLAN.md, R5/R6):

* the 15 V2 calibration engines are preserved and remain unused for model
...
* each fold's validation manifest uses the balanced V2.1 lifecycle fractions
...
from rul_prediction.data.pseudo_test import V2_1_LIFECYCLE_FRACTIONS
...
def cv_manifest_payload(dev_ids, calibration_ids, n_folds: int = N_FOLDS,
                        seed: int = SEED) -> dict:
    """Build the full V2.1 CV manifest payload (deterministic, JSON-serializable)."""
...
def write_v2_1_cv_manifest(dev_ids, calibration_ids, out_dir: str | Path,
                           n_folds: int = N_FOLDS, seed: int = SEED) -> Path:
    """Persist fd001_v2_1_group_cv_seed42.json and return its path."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / f"fd001_v2_1_group_cv_seed{seed}.json"
    payload = cv_manifest_payload(dev_ids, calibration_ids, n_folds, seed)
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    return path


def read_v2_1_cv_manifest(path: str | Path) -> dict:
    """Read the V2.1 CV manifest; returns the parsed payload dict."""
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    assert payload["methodology"] == "v2.1"
    return payload
