"""Shared Methodology M1 benchmark plumbing.

Used by ``scripts/run_m1_benchmark.py`` (primary w30 benchmark) and
``scripts/run_m1_ablation.py`` (window/hyperparameter ablations) so that every
...
from rul_prediction.features.m1_features import extract_m1_features
...
def load_m1_artifacts(dataset: str = "FD001", data_dir: str | Path = "data/raw",
                      splits_dir: str | Path = "experiments/splits") -> dict:
    """Load raw frame, M1 split, train-only scaler, manifest and validation trajectories."""
    frame = load_train(dataset, data_dir)
    split_path = Path(splits_dir) / f"{dataset}_m1_seed{SEED}.json"
    train_ids, validation_ids, calibration_ids = read_m1_split_file(split_path)
    m1_dir = ROOT / "data" / "processed" / "m1" / dataset / "raw"
    scaler = load_joblib(m1_dir / f"{dataset}_scaler.joblib")
    manifest = load_manifest(Path(splits_dir) / f"{dataset.lower()}_m1_validation_cutoffs.csv")
...
def partition_sequences(frame, engine_ids: set[int], scaler, window: int):
    ...
    return build_m1_train_sequences(scaled, rows["engine_id"].to_numpy(), rul, window)
...
def classical_features(X, engine_ids, n_observed, window: int):
    ...
    for engine in np.unique(engine_ids):
        ...
        for i, idx in enumerate(block):
            ...
            f, names = extract_m1_features(observed, cutoff_cycle=i + 1)
            feats.append(f)
...
def train_model(model_name: str, window: int, n_features: int, overrides: dict | None,
    ...
    if model_name in ("lstm", "gru", "tcn"):
        ...
        from rul_prediction.models.m1_models import m1_gru, m1_lstm, m1_tcn
        from rul_prediction.training.trainer import set_seed

        set_seed(seed)
        builder = {"lstm": m1_lstm, "gru": m1_gru, "tcn": m1_tcn}[model_name]
...
def make_predictor(model_name: str, model, scaler, *, window: int):
    ...
    if model_name in ("linear", "rf", "xgboost"):
        def predict_classical(history, cutoff):
            ...
            features, _ = extract_m1_features(observed, cutoff)
...
def evaluate_official_test(model_name: str, model, scaler, window: int,
    ...
    were inspected during the M1-0 audit. Results must be reported as
...
def run_experiment(experiment_id: str, model_name: str, window: int, overrides: dict | None = None,
    ...
    artifacts = load_m1_artifacts(dataset, data_dir, splits_dir)
