"""Methodology V2.2 nested engine-group CV (FD001) — training-control clean.

Repairs V2.2-1/V2.2-2/V2.2-3 (see V2_2_REPAIR_PLAN.md):
...
- 5 outer engine folds (seed 42, from the V2.1 manifest): per fold
...
- Inner early-stop split INSIDE the 68 outer-training engines:
  ...
  V2_2_REPAIR_PLAN.md before any V2.2 result was inspected.
...
from rul_prediction.data.pseudo_test import V2_1_LIFECYCLE_FRACTIONS, load_manifest
...
def inner_early_stop_split(outer_train_ids: set[int], fold: int,
    ...
    Documented scheme (V2_2_REPAIR_PLAN.md): random.Random(4200+fold) shuffles
...
def load_v2_2_cv_artifacts(dataset: str = "FD001", data_dir: str | Path = "data/raw",
                           splits_dir: str | Path = "experiments/splits") -> dict:
    """Frame + V2.1 CV manifest payload (dev/cal split + 5 outer folds)."""
    frame = load_train(dataset, data_dir)
    payload = read_v2_1_cv_manifest(
...
def _stage1_deep(candidate, frame, inner_fit, inner_stop, fold, *,
    ...
    builder = v2_gru if candidate["model"] == "gru" else v2_lstm
    set_seed(seed)
...
def apply_selection_policy(summary: list[dict], n_folds: int = 5) -> dict:
    """Pre-declared V2.2 selection rule (see V2_2_REPAIR_PLAN.md).
