"""Methodology M2 CV benchmark plumbing (engine-group 5-fold cross-validation).

See M2_REPAIR_PLAN.md (R5-R8). Key differences vs M1:

- development/calibration engine separation: the 15 M1 calibration engines
...
def load_m2_cv_artifacts(dataset: str = "FD001", data_dir: str | Path = "data/raw",
                           splits_dir: str | Path = "experiments/splits") -> dict:
    """Frame + M2 CV manifest payload (validation + training ids per fold)."""
    frame = load_train(dataset, data_dir)
    manifest_payload = read_m2_cv_manifest(
...
def run_cv_fold(candidate: dict, fold: dict, frame, data_dir: str | Path,
    ...
    manifest = load_manifest(Path(splits_dir) /
                             f"fd001_m2_fold{fold['fold']}_validation_cutoffs.csv")
