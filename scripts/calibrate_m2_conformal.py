"""Methodology M2: engine-cluster conformal calibration (FD001).
...
The frozen model (models/m2/fd001_gru_w45_huber.keras) is evaluated on the
...
Outputs:
    reports/tables/m2_conformal_calibration.csv   (15 engine scores + q)
    reports/tables/m2_conformal_coverage.csv      (held-out empirical coverage)
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M2 engine-level conformal calibration")
    parser.add_argument("--data-dir", default="data/raw")
    parser.add_argument("--splits-dir", default="experiments/splits")
    args = parser.parse_args()

    model = keras.models.load_model(ROOT / "models/m2/fd001_gru_w45_huber.keras")
    scaler = load_joblib(ROOT / "models/m2/fd001_scaler.joblib")
    window = 45
    predictor = make_predictor("gru", model, scaler, window=window)

    frame = load_train("FD001", args.data_dir)
    cal_ids = pd.read_csv(Path(args.splits_dir) / "fd001_m2_calibration_cutoffs.csv")
    trajectories = {
        int(e): g.sort_values("cycle")
        for e, g in frame[frame["engine_id"].isin(cal_ids["engine_id"].unique())]
        .groupby("engine_id")
    }
    cal_manifest = load_manifest(Path(args.splits_dir) / "fd001_m2_calibration_cutoffs.csv")
    ...
    with open("reports/tables/m2_conformal_calibration.csv", "w", newline="",
              encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=["engine_id", "max_abs_error", "n_checkpoints"])
        writer.writeheader()
        writer.writerows(score_rows)
    pd.DataFrame(coverage_rows).to_csv("reports/tables/m2_conformal_coverage.csv", index=False)
    pd.DataFrame({"alpha": list(qs), "q": list(qs.values())}).to_csv(
        "reports/tables/m2_conformal_q.csv", index=False)
    Path("experiments/m2/conformal_calibration.json").write_text(
