"""Methodology V2 raw-RUL preprocessing pipeline.

Builds the canonical V2 processed artifacts (target_mode = raw by default):
    data/processed/v2/FD001/raw/FD001_train_sequences.npz   (padded windows + raw RUL)
    data/processed/v2/FD001/raw/FD001_scaler.joblib          (fit on 70 TRAIN engines only)
    data/processed/v2/FD001/raw/FD001_metadata.json

Secondary experiment (optional, never the V2 default):
    --target-mode capped --cap 45  ->  data/processed/v2/FD001/capped45/

Legacy V1 artifacts under data/processed/FD001_* are untouched.

Usage:
    .venv/Scripts/python.exe scripts/preprocess_v2.py --dataset FD001 [--window 30]
    .venv/Scripts/python.exe scripts/preprocess_v2.py --dataset FD001 --target-mode capped --cap 45
"""
...
OUT_ROOT = Path("data/processed/v2")


def main() -> None:
    parser = argparse.ArgumentParser(description="Methodology V2 raw-RUL preprocessing")
    ...
    parser.add_argument("--target-mode", choices=TARGET_MODES, default="raw",
                        help="explicit target definition; V2 default is raw (never inferred from paths)")
    ...
    if not split_path.exists():
        raise SystemExit(
            f"V2 split file not found: {split_path}. Run: "
            ".venv/Scripts/python.exe scripts/build_v2_manifests.py --dataset %s" % args.dataset
        )
    ...
    X_train, y_train, train_seq_ids, n_observed, masks = build_v2_train_sequences(
