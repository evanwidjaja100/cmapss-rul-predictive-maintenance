"""Methodology M1 primary raw-RUL benchmark (window 30, no cap).

Runs mean/linear/random-forest/XGBoost/LSTM/GRU/TCN with IDENTICAL:
    - train partition (70 M1 engines), scaler (train-only), raw-RUL target
    - fixed 75-row validation pseudo-test manifest (every model, same rows)
Writes:
    experiments/m1_results.csv              (one row per model, idempotent)
    experiments/m1_validation_predictions.csv (raw predictions per manifest row)
The official FD001 test set is NOT contacted here.

Usage:
    .venv/Scripts/python.exe scripts/run_m1_benchmark.py
    .venv/Scripts/python.exe scripts/run_m1_benchmark.py --models lstm gru tcn
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Methodology M1 raw-RUL benchmark")
    ...
    for model_name in args.models:
        experiment_id = f"m1_{model_name}_w{args.window}_s42"
