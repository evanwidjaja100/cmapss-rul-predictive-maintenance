"""Methodology M3: engine-cluster conformal calibration on the CLEAN final model.

Runs only after the M3 final fit (scripts/run_m3_freeze.py) — the 15
calibration engines' labels are touched HERE, after fitting is complete
(M3_REPAIR_PLAN.md M3-1/M3-5). The M3 model fit and model selection
...
Outputs:
    ...
    reports/tables/m3_conformal_*.csv
    experiments/m3/conformal_calibration.json
"""
...
def calibration_scores(model, scaler, window: int, candidate: str) -> pd.DataFrame:
    ...
    cal = pd.read_csv(SPLITS_DIR / "fd001_m3_calibration_cutoffs.csv")
    ...
    manifest = load_manifest(SPLITS_DIR / "fd001_m3_calibration_cutoffs.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M3 conformal calibration")
    ...
    import yaml
    cfg = yaml.safe_load(Path("configs/final_model_m3_fd001.yaml").read_text(encoding="utf-8"))
    ...
    scaler_posix = "models/m3/fd001_scaler.joblib"
    if arch.startswith(("gru", "lstm")):
        model_posix = f"models/m3/fd001_{candidate}.keras"
        ...
        from joblib import load as load_joblib
        model_posix = f"models/m3/fd001_{candidate}.joblib"
        model_path = ROOT / model_posix
        verify_before_load(model_posix, root=ROOT, manifest_dataset="FD001")
        verify_before_load(scaler_posix, root=ROOT, manifest_dataset="FD001")
    ...
    pd.DataFrame(coverage_rows).to_csv("reports/tables/m3_conformal_coverage.csv", index=False)
    ...
    result = {
        ...
        "historical_caveat": ("The calibration engines were held out from M3 fitting and model "
