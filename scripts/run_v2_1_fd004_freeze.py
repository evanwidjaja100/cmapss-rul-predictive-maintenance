"""Methodology V2.1: freeze the FD004 condition-aware model (variant C).
...
Post-hoc official evaluation: FD004 labels were inspected in V2-11
(reports/v2_fd004.md); official metrics are post-hoc, never sealed.

Artifacts:
    configs/final_model_v2_1_fd004.yaml
    models/v2_1/fd004_gru_w45_huber_condC.keras
    models/v2_1/fd004_conditionC.joblib  (kmeans, cluster_scalers, settings_scaler)
    reports/tables/v2_1_fd004_official.csv
    reports/tables/v2_1_fd004_predictions.csv
"""
...
from run_v2_1_fd004 import build_matrix  # exact variant-C feature pipeline

WINDOW = 45


def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze V2.1 FD004 condition-C model")
    ...
    set_seed(42)
    model = v2_gru(WINDOW, X.shape[1], loss="huber", seed=42)
    ...
    metrics = {
        "methodology": "v2.1", "dataset": "FD004", "variant": "C",
        "model": "gru_w45_huber_condC",
        "official_status": "post-hoc (labels inspected in V2-11; never sealed)",
    ...
    pd.DataFrame([metrics]).to_csv("reports/tables/v2_1_fd004_official.csv", index=False)
    pd.DataFrame({"engine_id": engines, "true_rul_official": rul,
                  "prediction": pred}).to_csv(
        "reports/tables/v2_1_fd004_predictions.csv", index=False)
    Path("experiments/v2_1/fd004_final_model_metrics.json").write_text(
