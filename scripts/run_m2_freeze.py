"""Methodology M2: freeze the CV-selected FD001 model and evaluate post-hoc.

Reads configs/final_model_m2_fd001.yaml (source of truth for every training
...
Artifacts:
    models/m2/fd001_gru_w45_huber.keras
    models/m2/fd001_scaler.joblib
    reports/tables/m2_fd001_official.csv
    experiments/m2/fd001_final_model_metrics.json

Integrity checks: config's CV metrics must match experiments/m2/fd001_cv_summary.csv,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze M2 FD001 final model")
    parser.add_argument("--config", default="configs/final_model_m2_fd001.yaml")
    parser.add_argument("--data-dir", default="data/raw")
    parser.add_argument("--splits-dir", default="experiments/splits")
    args = parser.parse_args()

    cfg = yaml.safe_load(Path(args.config).read_text(encoding="utf-8"))
    assert cfg["methodology"] == "m2" and cfg["dataset"] == "FD001"
    model_cfg = cfg["model"]

    cv_summary = pd.read_csv("experiments/m2/fd001_cv_summary.csv")
    ...
    payload = read_m2_cv_manifest(Path(args.splits_dir) / "fd001_m2_group_cv_seed42.json")
    ...
    metrics.update({
        ...
        "notes": notes,
        "official_status": "post-hoc (labels inspected in M1-0 audit; never blind)",
    ...
    pd.DataFrame([metrics]).to_csv("reports/tables/m2_fd001_official.csv", index=False)
    pd.DataFrame(prediction_rows).to_csv("reports/tables/m2_fd001_predictions.csv", index=False)
    Path("experiments/m2/fd001_final_model_metrics.json").write_text(
