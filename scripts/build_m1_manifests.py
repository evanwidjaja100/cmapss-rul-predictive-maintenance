"""Generate Methodology M1 engine split and fixed pseudo-test manifests.

Creates (all deterministic, seed 42, never overwrites legacy split files):
    experiments/splits/FD001_m1_seed42.json           (70/15/15 engine split)
    experiments/splits/fd001_m1_validation_cutoffs.csv (75 pseudo-test rows)
    experiments/splits/fd001_m1_calibration_cutoffs.csv (75 pseudo-test rows)

Usage:
    .venv/Scripts/python.exe scripts/build_m1_manifests.py --dataset FD001
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Methodology M1 split + pseudo-test manifests")
    ...
    split_path = write_m1_split_file(engine_ids, args.dataset, args.out_dir, args.seed)
    ...
    val_path = save_manifest(
        validation_manifest, out_dir / f"{args.dataset.lower()}_m1_validation_cutoffs.csv")
    cal_path = save_manifest(
        calibration_manifest, out_dir / f"{args.dataset.lower()}_m1_calibration_cutoffs.csv")
