"""Methodology V2.1: honest error analysis of the frozen FD001 model.

Corrects the V2 error analysis (V2_1_REPAIR_PLAN.md R1/R2/R4): official C-MAPSS
...
Outputs: reports/tables/v2_1_error_profile.csv, reports/v2_1_error_analysis.md
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V2.1 error analysis (FD001, frozen model)")
    parser.add_argument("--data-dir", default="data/raw")
    args = parser.parse_args()

    preds = pd.read_csv("reports/tables/v2_1_fd001_predictions.csv")
    ...
    for g, label in ((padded, "padded_observed_lt_45"), (full, "full_observed_ge_45")):
        ...
        rows.append(s)
    profile = pd.DataFrame(rows)
    profile.to_csv("reports/tables/v2_1_error_profile.csv", index=False)
    ...
    md = [
        "# V2.1 Error Analysis - frozen FD001 model (post-hoc official test)",
        "",
        f"Model: `gru_w45_huber` (V2.1 CV-selected, 85 development engines). "
        f"Official test: {len(df)} engines. Total NASA = {total_nasa:.2f}.",
        "",
        "**Terminology correction (V2_1_REPAIR_PLAN.md R1/R2):** official test "
    ...
    report = "\n".join(md)
    Path("reports/v2_1_error_analysis.md").write_text(report, encoding="utf-8")

    summary = {"total_NASA": total_nasa,
               "padded_observed_lt_window_count": int((df["observed_cycles"] < WINDOW).sum())}
    Path("experiments/v2_1/error_analysis.json").write_text(
        json.dumps(summary, indent=2), encoding="utf-8")
    print(profile.to_string(index=False))
    print(f"\nReport -> reports/v2_1_error_analysis.md (NASA total {total_nasa:.2f})")
