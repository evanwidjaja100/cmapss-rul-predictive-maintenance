"""Phase M1-6: residual/error analysis of the frozen M1 model.

Loads the frozen model (models/m1_frozen_gru_w45_huber.keras) and profiles
per-engine prediction errors on both evaluation samples:

* validation manifest  — 75 fixed rows (the model-selection sample)
* official FD001 test  — 100 engines, POST-HOC (labels inspected in M1-0 audit)

Outputs: experiments/m1_error_profile.csv (per-engine rows, gitignored) and a
printed summary used to author reports/m1_error_analysis.md. No training and
...
def main() -> None:
    ...
    print(f"train raw RUL (14,235 seqs): mean 106.7 median 101 max 361  (M1-2 stats)")
