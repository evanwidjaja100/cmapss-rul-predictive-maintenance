"""Methodology M3: post-hoc official FD004 evaluation (after YAML freeze).

Official FD004 labels are permanently POST-HOC (inspected in M1-11; never
...
try:
    from run_m3_fd004 import build_matrix, make_predictor
except ImportError:
    from scripts.run_m3_fd004 import build_matrix, make_predictor

OUT_DIR = ROOT / "experiments" / "m3"
...
def load_and_verify_condition(config, *, model=None) -> dict:
    ...
    if not cond_path.exists():
        raise FileNotFoundError(
            f"FD004 condition artifact missing: {cond_path}. Generate via scripts/run_m3_fd004_freeze.py "
            f"(or ensure models/m3/fd004_condition{config.selected_variant}.joblib exists)"
        )
...
def main() -> None:
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"FD004 model artifact missing: {model_path}. Generate via scripts/run_m3_fd004_freeze.py "
            f"(or ensure models/m3/fd004_gru_w{window}_{config.loss}_cond{variant}.keras exists)"
        )
    if not cond_path.exists():
        raise FileNotFoundError(
            f"FD004 condition artifact missing: {cond_path}. Generate via scripts/run_m3_fd004_freeze.py"
    ...
    metrics = {
        ...
        "dataset": "FD004",
        "variant": variant,
        "official_status": "post-hoc (FD004 official labels permanently post-hoc; never select or tune M3)",
    ...
    pd.DataFrame([metrics]).to_csv(ROOT / "reports" / "tables" / "m3_fd004_official.csv", index=False)
    pd.DataFrame({"engine_id": engines, "true_rul_official": y_true,
                  "prediction": pred}).to_csv(
        ROOT / "reports" / "tables" / "m3_fd004_predictions.csv", index=False)
