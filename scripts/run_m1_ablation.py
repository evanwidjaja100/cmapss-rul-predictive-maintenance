"""Methodology M1 controlled ablations (target definition is NEVER changed).
...
Results append to experiments/m1_results.csv (idempotent) and the curated
table is written to reports/tables/m1_ablation_results.csv.

Usage:
    .venv/Scripts/python.exe scripts/run_m1_ablation.py            (full matrix)
    .venv/Scripts/python.exe scripts/run_m1_ablation.py --window 15 --models rf xgboost
"""
...
def _experiment_id(model: str, window: int, overrides: dict) -> str:
    ...
    return f"m1_{model}_w{window}{suffix}_s42"


def main() -> None:
    parser = argparse.ArgumentParser(description="Methodology M1 controlled ablations")
    ...
    out_csv = ROOT / "experiments" / "m1_results.csv"
    pred_csv = ROOT / "experiments" / "m1_validation_predictions.csv"
    done_ids = set()
    if out_csv.exists():
    ...
    table = ROOT / "reports" / "tables" / "m1_ablation_results.csv"
