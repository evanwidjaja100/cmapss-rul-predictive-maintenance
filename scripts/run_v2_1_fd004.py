"""Methodology V2.1: FD004 condition-aware experiments A/B/C/D (issue R13).

Same split (175 dev / 37 val / 37 cal), target (raw RUL), manifest
(fd004_v2_1_validation_cutoffs.csv) and training recipe (GRU w45 huber,
epochs 60, batch 256, patience 8, seed 42) for all variants:

    A  global scaler, 21 sensor inputs            (V2-11 baseline reproduction)
...
Success (V2_1_REPAIR_PLAN.md R13): prediction variance no longer collapses,
RMSE decreases and R2 improves vs A.

Results append idempotently to experiments/v2_1/fd004_variant_results.csv and
experiments/v2_1/fd004_variant_predictions.csv.

Usage: python scripts/run_v2_1_fd004.py [--variants A B C D]
"""
