"""Phase M1-7: explainability of the frozen model via sensor-level attribution.
...
distribution. (The script was renamed from explain_m1_shap.py in M2 because
it is not SHAP; keras 3 breaks shap's deep/kernel explainers on this model —
see reports/m1_explainability.md.) Attributions are in raw-RUL units
...
Target groups (from experiments/m1_error_profile.csv, full windows only):
...
Writes experiments/m1_shap_attributions.csv and
experiments/m1_shap_position_engine67.csv (gitignored). No training, no
calibration contact.
"""
...
from rul_prediction.benchmark.m1 import ROOT, load_m1_artifacts
from rul_prediction.data.loader import SENSOR_COLUMNS, load_test
from rul_prediction.data.preprocessing import transform
from rul_prediction.data.m1_preprocessing import build_m1_train_sequences

WINDOW = 45
N_SENSORS = len(SENSOR_COLUMNS)
MODEL_PATH = ROOT / "models" / "m1_frozen_gru_w45_huber.keras"
PROFILE_CSV = ROOT / "experiments" / "m1_error_profile.csv"
OUT_SENSOR = ROOT / "experiments" / "m1_shap_attributions.csv"
OUT_POSITION = ROOT / "experiments" / "m1_shap_position_engine67.csv"
...
def load_windows() -> tuple[np.ndarray, dict]:
    ...
    X, y, ids, n_obs, masks = build_m1_train_sequences(
