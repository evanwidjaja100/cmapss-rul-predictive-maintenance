"""Methodology M2 bounded model-selection CV (FD001, 5 engine-group folds).
...
candidates x 5 folds) and appends results idempotently to:
    ...
    experiments/m2/fd001_cv_fold_results.csv
    experiments/m2/fd001_cv_engine_level.csv
    experiments/m2/fd001_cv_predictions.csv
    experiments/m2/fd001_cv_summary.csv     (mean +/- std over folds)

Usage:
    python scripts/run_m2_cv.py                          # all candidates, all folds
    python scripts/run_m2_cv.py --candidates gru_w45_huber lstm_w45_huber
    python scripts/run_m2_cv.py --folds 1 2
"""
...
from rul_prediction.benchmark.m2 import (
    CV_CANDIDATES,
    cv_summary,
    load_m2_cv_artifacts,
    run_cv_fold,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M2 engine-group CV model selection")
    ...
    artifacts = load_m2_cv_artifacts("FD001", args.data_dir, args.splits_dir)
