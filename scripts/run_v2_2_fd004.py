...
def make_predictor(variant: str, model, kmeans, cluster_scalers, settings_scaler,
    ...
    def predict_one(history, cutoff):
        ...
        # verify model input shape vs config
        # Keras model input 0 is (None, window, n_features)
        try:
            expected = model.input_shape[0]  # type: ignore
            if expected[1] != window or expected[2] != n_features:
                raise ValueError(
                    f"model input {expected} disagrees with window {window} features {n_features}"
                )
        except Exception as ex:
            # if model has no input_shape (stub), check via layer
            if "disagrees" in str(ex):
                raise
            pass
...
def train_epochs(variant: str, frame: pd.DataFrame, fit_ids: set[int],
    ...
    set_seed(seed)
    model = v2_gru(window, X.shape[1], units=units, dropout=dropout, loss=loss, seed=seed, learning_rate=lr)
    callbacks = []
...
def main() -> None:
    ...
    if best_rows:
        ...
        if not existing_best.empty:
            ...
            # reject duplicates already handled
        for c in best_rows:
            # if variant already existed we are adding new variant; no overwrite of prior rows except for new variants
            # if prior file had variant and we are merging same variant (should have been skipped), raise
            if c["variant"] in merged and c["variant"] not in {b["variant"] for b in best_rows}:
                # shouldn't happen because we skipped done variants
                pass
            merged[c["variant"]] = c
        # validate no duplicates after merge
        if len(merged) != len(set(merged.keys())):
            raise ValueError("duplicate variant in best_epochs merge")
    ...
    if not results.empty:
        ...
        is_canonical = Path(args.config_out) == Path("configs/final_model_v2_2_fd004.yaml")
...
def write_fd004_config(winner: pd.Series, results: pd.DataFrame,
    ...
    if Path(config_out) == Path("configs/final_model_v2_2_fd004.yaml"):
    ...
    # atomic write
    import tempfile, os
    tmp = Path(tempfile.mktemp(dir=str(out.parent)))
