"""Methodology M3: FD004 condition-aware variant comparison, training-control clean.

Repairs M3-8: the 37 outer validation engines must NEVER influence training
...
RMSE, then |signed bias| (pre-declared, M3_REPAIR_PLAN.md). Official FD004
labels remain POST-HOC forever and never select the variant.

Outputs:
    experiments/m3/fd004_variant_results.csv
    experiments/m3/fd004_variant_predictions.csv
    experiments/m3/fd004_best_epochs.csv
"""
...
OUT_DIR = REPO_ROOT / "experiments" / "m3"
VARIANTS = {"A", "B", "C", "D"}
...
def train_epochs(variant: str, frame: pd.DataFrame, fit_ids: set[int],
    ...
    set_seed(seed)
    model = m1_gru(window, X.shape[1], units=units, dropout=dropout, loss=loss, seed=seed,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FD004 M3 variant comparison")
    ...
    parser.add_argument("--config-out", default=str(REPO_ROOT / "configs" / "final_model_m3_fd004.yaml"))
    ...
    if not results.empty:
        ...
        canonical_path = (REPO_ROOT / "configs" / "final_model_m3_fd004.yaml").resolve()
...
def write_fd004_config(winner: pd.Series, results: pd.DataFrame,
    ...
    """Generate configs/final_model_m3_fd004.yaml from the variant results.
    ...
    split = json.loads((REPO_ROOT / "experiments" / "splits" / "FD004_m1_seed42.json").read_text(encoding="utf-8"))
    # raw exact-file evidence hashes for the split JSON and validation cutoff CSV
    raw_hashes = compute_split_evidence_hashes(
        "experiments/splits/FD004_m1_seed42.json",
        "experiments/splits/fd004_m2_validation_cutoffs.csv",
        root=REPO_ROOT,
    )
    ...
    cfg = {
        ...
        "splits": {
            "provenance": "experiments/splits/FD004_m1_seed42.json",
            ...
            "validation_manifest": "experiments/splits/fd004_m2_validation_cutoffs.csv",
        },
        "training": {
