"""Methodology M3: build fixed FD001 split/outer-eval/calibration manifests.

The outer folds reuse the M2 engine-group manifest (seed 42, fixed before any
model comparison). M3 adds the deterministic inner early-stop split scheme
(58 inner-fit / 10 inner-stop, seeds 4201..4205) and writes M3-namespaced
pseudo-test manifests with canonical hashes.

Outputs:
    experiments/splits/fd001_m3_outer_fold{1..5}_cutoffs.csv   (17 engines x 5)
    experiments/splits/fd001_m3_calibration_cutoffs.csv        (15 engines x 5)
    experiments/m3/fd001_outer_split_manifest.json             (ids + hashes)
"""
...
from rul_prediction.data.pseudo_test import M2_LIFECYCLE_FRACTIONS, build_pseudo_test_manifest
...
OUT_DIR = Path("experiments/m3")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build M3 FD001 manifests")
    ...
    payload = read_m2_cv_manifest(splits_dir / "fd001_m2_group_cv_seed42.json")
    ...
    for f in payload["folds"]:
        ...
        manifest = build_pseudo_test_manifest(
            {e: lifetimes[e] for e in val_ids}, M2_LIFECYCLE_FRACTIONS)
        path = splits_dir / f"fd001_m3_outer_fold{f['fold']}_cutoffs.csv"
        manifest.to_csv(path, index=False)
        legacy = splits_dir / f"fd001_m2_fold{f['fold']}_validation_cutoffs.csv"
        if legacy.exists():
            assert (pd.read_csv(legacy, dtype={"engine_id": int}).sort_values(
                ...
                f"M3 outer fold {f['fold']} must equal the fixed M2 validation manifest")
    ...
    cal_manifest = build_pseudo_test_manifest(
        {e: lifetimes[e] for e in cal_ids}, M2_LIFECYCLE_FRACTIONS)
    cal_path = splits_dir / "fd001_m3_calibration_cutoffs.csv"
    cal_manifest.to_csv(cal_path, index=False)
    legacy_cal = splits_dir / "fd001_m2_calibration_cutoffs.csv"
    ...
    from rul_prediction.benchmark.m3 import INNER_FIT_SIZE, INNER_STOP_SIZE, inner_early_stop_split
    inner = {}
    for f in payload["folds"]:
