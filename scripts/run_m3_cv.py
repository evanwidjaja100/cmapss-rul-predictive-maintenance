"""Methodology M3: nested engine-group CV (FD001) — 8 candidates x 5 folds.
...
Usage:
    python scripts/run_m3_cv.py                      # all 8 candidates x 5 folds
    python scripts/run_m3_cv.py --candidates gru_w45_huber --folds 1
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M3 nested CV (FD001)")
    ...
    artifacts = load_m3_cv_artifacts("FD001", args.data_dir, args.splits_dir)
    ...
    if args.candidates is None and args.folds is None:
        assert n == 40, "full M3 matrix must be exactly 8 candidates x 5 folds = 40"
    ...
    print("\nM3 CV summary (mean +/- std over outer folds):")
