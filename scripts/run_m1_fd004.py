"""Phase M1-11: FD004 generalization — replicate the M1 methodology on FD004.

Reuses the exact M1 recipe from the FD001 freeze (GRU, window 45, huber,
...
baseline recorded at download time (M1-11, 2026-08-15). The repository
...
Writes:
    models/m1_fd004_gru_w45_huber.keras          (gitignored)
    reports/tables/m1_fd004_official.csv         (tracked)
    experiments/m1_fd004_official_predictions.csv (gitignored)
"""
...
MODEL_PATH = ROOT / "models" / "m1_fd004_gru_w45_huber.keras"
RESULT_CSV = ROOT / "reports" / "tables" / "m1_fd004_official.csv"
PRED_CSV = ROOT / "experiments" / "m1_fd004_official_predictions.csv"


def check_sealed_rul() -> None:
    """Label integrity pin (historical). The 'sealed/first-ever' status ended at
    M1-11 when the labels were inspected; official FD004 is now post-hoc by
    policy (M2_REPAIR_PLAN.md R14). The hash pin remains as an integrity check.
    """
...
def main() -> None:
    ...
    artifacts = load_m1_artifacts(DATASET)
    ...
    row = {
        "experiment_id": "m1_fd004_gru_w45_huber",
        "model": MODEL_NAME,
        "target_mode": "raw",
        "window": WINDOW,
