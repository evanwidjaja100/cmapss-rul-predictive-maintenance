"""Methodology M3: post-hoc official FD001 evaluation (STRICTLY after CV ->
selection -> YAML freeze -> conformal calibration).

The official labels are permanently POST-HOC (inspected in the M1-0 audit) and
never select or tune the M3 model. This script only reads the frozen model.
...
from rul_prediction.benchmark.m3 import CV_CANDIDATES, apply_selection_policy, cv_summary
...
def main() -> None:
    ...
    if model_name in ("rf", "xgboost"):
        verify_before_load(f"models/m3/fd001_{candidate}.joblib", root=ROOT, manifest_dataset="FD001")
        ...
        verify_before_load(f"models/m3/fd001_{candidate}.keras", root=ROOT, manifest_dataset="FD001")
        verify_before_load("models/m3/fd001_scaler.joblib", root=ROOT, manifest_dataset="FD001")
    ...
    metrics = {
        ...
        "dataset": "FD001",
        "candidate": candidate,
        "official_status": "post-hoc (FD001 official labels permanently post-hoc; never select or tune M3)",
    ...
    pd.DataFrame([metrics]).to_csv("reports/tables/m3_fd001_official.csv", index=False)
    pd.DataFrame({"engine_id": engines, "true_rul_official": y_true,
                  "prediction": pred}).to_csv(
        "reports/tables/m3_fd001_predictions.csv", index=False)
