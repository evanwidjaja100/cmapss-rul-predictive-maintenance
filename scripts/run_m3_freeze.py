"""Methodology M3: freeze the CV-selected FD001 model (YAML-driven, clean fit).

Training protocol (M3_REPAIR_PLAN.md M3-1, M3-13):

    1. READ every configurable value from configs/final_model_m3_fd001.yaml
    ...
    5. Save model + scaler + metadata. NO official evaluation here: post-hoc
       metrics are generated by scripts/run_m3_posthoc.py AFTER conformal
       calibration (scripts/run_m3_conformal.py).

Artifacts:
    models/m3/fd001_<candidate>.keras
    models/m3/fd001_scaler.joblib
    experiments/m3/fd001_final_fit_metadata.json
"""
...
def final_fit_plan(cfg: dict, frame: pd.DataFrame) -> dict:
    """Resolve the clean final-fit plan; asserts the M3 calibration-isolation invariant.
...
def fit_final_model(plan: dict, frame: pd.DataFrame, data_dir: str | Path):
    ...
    from rul_prediction.benchmark.m1 import classical_features
    ...
    if arch == "rf":
        ...
        set_seed(plan["seed"])
        builder = m1_gru if arch == "gru" else m1_lstm
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze M3 FD001 final model")
    ...
    Path("experiments/m3/fd001_final_fit_metadata.json").write_text(
