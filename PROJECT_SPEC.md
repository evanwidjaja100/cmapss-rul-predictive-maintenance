...
| tests | CI-verified artifact-free tiers (`static_contract`, `unit`, `tracked_artifacts`, `integration`, `app_smoke`; aggregate `not needs_artifacts` guard); historical measurements are dated snapshots in the implementation report, not permanent counts; `tests/test_v2_2_protocol.py` (Tests A–H) |
...
| 14 testing / CI | V2-10 (`tests/`, `.github/workflows/ci.yml`) | done (historical counts preserved with date/commit; current health via CI tiers and reproducible commands, not permanent numeric claims) |
